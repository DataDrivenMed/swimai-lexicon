...
function updateSidebarVisibility() {
  /* On mobile we always show the sidebar (it's compact, stacked above content)
     On desktop we hide it outside Lexicon view */
  const mobile = window.innerWidth <= 860;
  if (mobile) {
    els.sidebar.classList.remove("sidebar--hidden");
    els.appShell.classList.remove("no-sidebar");
    return;
  }
